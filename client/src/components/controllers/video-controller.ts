...
export class VideoController {
  private readonly element: HTMLVideoElement;
  private isPausingBlocked: boolean;
  private readonly playingStateListeners: (() => Promise<void>)[] = [];
  private readonly volumeChangeListeners: (() => Promise<void>)[] = [];
  private readonly seekingListeners: (() => Promise<void>)[] = [];
  private cachedVolume: number;

  private readonly boundEmitPlayingStateChangeListener: () => void;
  private readonly boundEmitVolumeChangeListeners: () => void;
  private readonly boundEmitSeekingListeners: () => void;
  ...
  unmute() {
    ...
    this.element.volume = this.cachedVolume || 0.5;
  }

  updateVolume(percent: number) {
    const volume = Math.max(0, Math.min(1, percent / 100));
    this.element.muted = false; // Đảm bảo unmute trước khi thay đổi volume
