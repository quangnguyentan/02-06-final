...
interface FilterBarVideoReelsProps {
  currentCategory?: string;
}

const FilterBarVideoReels: React.FC<FilterBarVideoReelsProps> = ({
  currentCategory,
}) => {
  const categories = [
    "Tất cả",
  ...
  const searchPlaceholder =
    currentCategory && currentCategory !== "Tất cả"
      ...
      : "Tìm kiếm tên video, giải đấu, BLV...";

  return (
    <div className="my-4 rounded-lg shadow hidden md:flex flex-col sm:flex-row items-center space-y-2 sm:space-y-0 sm:space-x-3">
...
export default FilterBarVideoReels;
